    ...
    <div className={`fixed inset-0 z-50 transition-opacity duration-300 ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`} aria-hidden={!open}>
        ...
        <div className={`fixed left-0 right-0 bottom-0 top-0 z-60 bg-white transform transition-transform duration-300 ease-out ${open ? "translate-y-0" : "translate-y-full"} overflow-auto`}>
          <div className="flex items-center justify-between p-3 border-b sticky top-0 bg-white z-10">
            ...
            <button onClick={onClose} className="p-2 text-black hover:bg-gray-100 ">
              <PiX size={20} />
            </button>
          </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="mb-4 flex items-end gap-4">
                <div className="flex-1">
                  ...
                  <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)} className="w-full border border-gray-300 px-2 py-1  text-black text-sm">
                    {product.types.map((t) => (
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="px-3 py-1 bg-gray-200  text-black">−</button>
                    <div className="w-10 text-center text-black font-medium">{quantity}</div>
                    <button onClick={() => setQuantity((q) => q + 1)} className="px-3 py-1 bg-gray-200  text-black">+</button>
                  </div>
                </div>
              </div>

              <div className="flex gap-3">
                <button onClick={handlePayClick} disabled={loading} className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2  text-sm">
                  <PiShoppingCart />
                  {loading ? "Processing…" : `Pay ${product.priceMin * quantity} XAF`}
                </button>

                <button onClick={onClose} className="flex-1 px-4 py-2 border  text-black">Cancel</button>
              </div>
            </div>
          </div>
        </div>
